# "Thảo luận về bài viết trong X"

X is replaced by the parent forum channel name.

Full name example: `"Thảo luận về bài viết trong Chia Sẻ Kinh Nghiệm"`
...
Fired when a `message_sent` event occurs inside a thread whose parent channel
is of type `forum`, and the parent category is a tracked forum category.
The sender must be a non-bot user.

---

## Collection Logic

Same category-based forum routing as other forum events.
**GenericChannelProcessor (forum)** maps `message_sent` to the
`message_in_thread` interaction type and applies the pattern
`"Thảo luận về bài viết trong {channel_name}"`.
`channel_name` is resolved to the parent forum channel name.
