...
X is replaced by the thread (post) name.

Full name example: `"Xóa bài trong Thread Bài Viết"`
...
Fired when a `thread_deleted` event occurs in a thread whose parent channel is
a forum channel inside a tracked forum category.

---

## Collection Logic

**GenericChannelProcessor (forum)** maps `thread_deleted` to the
`thread_delete` interaction type and applies the pattern
`"Xóa bài trong {channel_name}"`. In this context `channel_name` resolves to
the thread/post name (not the parent forum name). Category tracking via
`TrackedForumCategories` ensures only forum channels in configured categories
fire this event.
