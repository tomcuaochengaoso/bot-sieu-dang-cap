# "Vào gif trong chat chung"

Event fired when a user sends a GIF in the general chat channel.
...
Fired when a `gif_sent` event occurs in the general chat channel
(`channel_id = 1092307876594323510`).

---

## Collection Logic

**GeneralChatHandler** maps the `"gif"` interaction type to this fixed event name.
GIF detection is handled upstream (in main.py or queue_handler.py) which
produces the `gif_sent` event type. GeneralChatHandler only routes events that
arrive in the exact general chat channel ID.
