...
Event fired when a user sends a regular message in the general chat channel (kênh tên là chat chung).
...
```json
{
  ...
  "general_chat_interaction": <boolean>  // Boolean flag marking this event as belonging to the general chat channel interaction set.
...
Fired when a `message_sent` event occurs in the general chat channel
(`channel_id = 1092307876594323510`) and the interaction type is `"message"`.

---

## Collection Logic

**GeneralChatHandler** is checked first in `try_specific_identification` before
the generic channel pipeline runs. It holds a hardcoded channel ID
(`1092307876594323510`) and a fixed mapping from interaction type to event name.
Only messages, GIFs, emojis, and stickers in this exact channel are routed here;
all other channels fall through to the generic pattern processors.
