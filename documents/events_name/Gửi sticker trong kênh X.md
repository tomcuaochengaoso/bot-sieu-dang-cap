# "Gửi sticker trong kênh X"

X is replaced by the channel name. 2 types: voice or text channel

Full name examples:
- `"Gửi sticker trong kênh Phòng Học"` (voice channel)
- `"Gửi sticker trong kênh tin-tuc"` (text channel)

This template applies to voice and text channels.
For forum channels the name is `"Gửi sticker trong X"` (no "kênh" prefix).
...
Fired when a `sticker_sent` event occurs in a voice or text channel.

---

## Collection Logic

**GenericChannelProcessor** resolves the event pattern `"Gửi sticker trong kênh {channel_name}"`.
For voice channels, `TrackedVoiceCategories` determines whether the channel is
fixed (luong = "Vào các kênh voice cố định") or custom (luong = "Vào các kênh voice tự tạo").
For text channels, the luong is always "Vào các kênh chat".
All voice and text channels are tracked by default — no channel-specific
