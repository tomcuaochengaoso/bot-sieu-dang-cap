# "Vào sticker trong chat chung"

Event fired when a user sends a sticker in the general chat channel.
...
Fired when a `sticker_sent` event occurs in the general chat channel
...
**GeneralChatHandler** maps the `"sticker"` interaction type to this fixed event
name. Only the exact general chat channel ID is routed here; stickers in any
other channel fall through to `"Gửi sticker trong kênh X"` or
`"Gửi sticker trong X"` depending on channel type.
